...
describe("B0.5 secret boundary", () => {
  ...
  it("persists consent metadata under owner-scoped RLS without secret-bearing columns", () => {
    ...
    expect(migration).not.toMatch(/^\s*(api_key|gemini_key|byok_key|credential|secret)\s+/im);
  });
