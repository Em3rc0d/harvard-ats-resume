export const ATS_RENDERER_CONTRACT_VERSION = "b9-ats-safe-single-column-v1" as const;
...
export function renderSemanticLinesText(lines: readonly ResumeSemanticLine[]): string {
  const blocks: string[] = [];
  let current: string[] = [];
...
function collectDocxHyperlinks(lines: readonly ResumeSemanticLine[]) {
  const urls: string[] = [];
...
function wordParagraph(line: ResumeSemanticLine, hyperlinkIds: ReadonlyMap<string, string>) {
  ...
  const runs = splitTextByUrls(sourceText).map((segment) => wordRun(
    segment.text, bold, size, segment.url ? (hyperlinkIds.get(segment.url) ?? null) : null,
  )).join("");
  return `<w:p><w:pPr>${spacing}</w:pPr>${runs}</w:p>`;
}

export function renderSemanticLinesDocx(linesInput: readonly ResumeSemanticLine[]): Uint8Array {
...
function wrapLine(text: string, width: number) {
  ...
  for (const word of words) {
    ...
    } else if (current.length === 0) current = word;
...
export function renderSemanticLinesPdf(linesInput: readonly ResumeSemanticLine[]): Uint8Array {
