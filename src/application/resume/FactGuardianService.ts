...
import type { CandidateResumeDocument, ResumeSourceRef, SourceBackedText } from "../../domain/resume/CandidateResumeDocument";
import {
...
import { buildProviderAttemptPlan, type AIExecutionProvenance } from "../ai/AIGatewayFoundation";
import {
  executeAICapability,
...
const HARMFUL = new Set(["POSSIBLE_NEW_CLAIM", "UNSUPPORTED_NEW_CLAIM", "SOURCE_CONFLICT"] as const);
...
type GuardianPassOutcome = Readonly<{
  ok: true;
  ...
  ok: false;
  failureCode: AIExecutionFailureCode | "FACT_GUARD_OUTPUT_INVALID" | "AI_ECONOMICS_POLICY_FAILED";
  attempts: readonly AIProviderAttemptReceipt[];
}>;
...
export type FactGuardianFailure = Readonly<{
  ok: false;
  failureCode: AIExecutionFailureCode | "FACT_GUARD_OUTPUT_INVALID" | "FACT_GUARD_REJECTED" | "AI_ECONOMICS_POLICY_FAILED";
  report: FactGuardianReport | null;
  attempts: readonly AIProviderAttemptReceipt[];
}>;
export type FactGuardianOutcome = FactGuardianSuccess | FactGuardianFailure;
export type FactGuardianConfig = AIGatewayRuntimeConfig & Readonly<{
...
function collectGeneratedUnits(document: GeneratedResumeDocument): GeneratedUnitDescriptor[] {
  ...
  const add = (path: string, unit: GeneratedResumeTextUnit | null) => { if (unit) units.push({ path, unit }); };
  if (document.header) {
    add("header.displayName", document.header.displayName);
    add("header.headline", document.header.headline);
    document.header.contactLines.forEach((unit, index) => add(`header.contactLines[${index}]`, unit));
  }
  ...
  document.certifications.forEach((entry, index) => collectEntry(`certifications[${index}]`, entry));
  ...
  document.languageGroups.forEach((group, index) => collectGroup(`languageGroups[${index}]`, group));
  document.otherGroups.forEach((group, index) => collectGroup(`otherGroups[${index}]`, group));
  return units;
}

function collectAllGeneratedOrdinals(document: GeneratedResumeDocument) {
  const values: number[] = [];
  const push = (refs: readonly ResumeSourceRef[]) => values.push(...refs.map((ref) => ref.ordinal));
  for (const unit of collectGeneratedUnits(document)) push(unit.unit.sourceRefs);
  if (document.header) push(document.header.sourceRefs);
  for (const entry of [...document.experience, ...document.projects, ...document.education, ...document.certifications]) push(entry.sourceRefs);
  for (const group of [...document.skillGroups, ...document.languageGroups, ...document.otherGroups]) push(group.sourceRefs);
...
function collectSourceFacts(document: CandidateResumeDocument) {
  ...
  const add = (unit: SourceBackedText | null) => {
    if (!unit) return;
    const key = `${unit.value}\n${sortedUniqueNumbers(unit.sourceRefs.map((ref) => ref.ordinal)).join(",")}`;
    if (seen.has(key)) return;
    seen.add(key);
    facts.push({ text: unit.value, sourceRefs: unit.sourceRefs });
  };
  if (document.identity) {
    add(document.identity.displayName); add(document.identity.headline); add(document.identity.location);
    add(document.identity.email); add(document.identity.phone); document.identity.links.forEach(add);
  }
  add(document.profile);
  document.employment.forEach((item) => {
    add(item.role); add(item.organization); add(item.startDateText); add(item.endDateText); add(item.location); add(item.summary);
    item.bullets.forEach(add); item.technologies.forEach(add);
  });
  document.projects.forEach((item) => {
    add(item.name); add(item.subtitle); add(item.url); add(item.summary); item.bullets.forEach(add); item.technologies.forEach(add);
  });
  document.education.forEach((item) => {
    add(item.institution); add(item.degree); add(item.field); add(item.startDateText); add(item.endDateText); add(item.location); item.notes.forEach(add);
  });
  document.certifications.forEach((item) => {
    add(item.name); add(item.issuer); add(item.dateText); add(item.credentialId); add(item.url);
  });
  document.skillGroups.forEach((item) => { add(item.label); item.skills.forEach(add); });
  document.languages.forEach((item) => { add(item.language); add(item.proficiency); });
  document.otherSections.forEach((item) => { add(item.heading); item.items.forEach(add); });
  return facts;
}

function actualOmittedOrdinals(source: CandidateResumeDocument, draft: GeneratedResumeDocument) {
  const used = collectAllGeneratedOrdinals(draft);
  const nonFactual = new Set(source.unassignedSourceOrdinals);
  return sortedUniqueNumbers(source.provenanceIndex.map((ref) => ref.ordinal).filter((ordinal) => !used.has(ordinal) && !nonFactual.has(ordinal)));
}
...
function buildGuardianPrompt(source: CandidateResumeDocument, draft: GeneratedResumeDocument) {
  ...
  return [
    ...
    "SOURCE FACTS:", JSON.stringify(sourceFacts),
    "GENERATED UNITS:", JSON.stringify(generatedUnits),
    "EXPECTED OMITTED SOURCE ORDINALS:", JSON.stringify(actualOmittedOrdinals(source, draft)),
  ].join("\n\n");
}
...
function guardianValidator(source: CandidateResumeDocument, draft: GeneratedResumeDocument) {
  ...
  return (value: unknown) => {
    ...
    if (!sameStrings(parsed.reviewedPaths, expectedPaths)) throw new Error("FACT_GUARD_PATH_COVERAGE_INVALID");
    if (!sameNumbers(parsed.reviewedSourceOrdinals, expectedSourceOrdinals)) throw new Error("FACT_GUARD_SOURCE_COVERAGE_INVALID");
    const generatedFindings = parsed.findings.filter((finding) => finding.classification !== "SOURCE_OMISSION");
    const generatedPaths = generatedFindings.map((finding) => finding.generatedPath).filter((path): path is string => path !== null);
    if (!sameStrings(generatedPaths, expectedPaths) || new Set(generatedPaths).size !== generatedPaths.length) {
      throw new Error("FACT_GUARD_FINDING_COVERAGE_INVALID");
    }
    for (const finding of parsed.findings) {
      if (finding.sourceOrdinals.some((ordinal) => !known.has(ordinal))) throw new Error("FACT_GUARD_REFERENCE_INVALID");
    ...
    if (!sameNumbers(omissionOrdinals, omitted)) throw new Error("FACT_GUARD_OMISSION_COVERAGE_INVALID");
  };
}
...
async function runGuardianPass(
  ...
  const outcome = await executeAICapability({
    ...
    systemInstruction: "You are CV Engine's independent Fact Guardian. The Editor has no authority to approve its own output. Compare generated meaning to candidate-authoritative source only, classify every factual unit, and return structured findings without rewriting the resume.",
  ...
  try {
    ...
    return { ok: false, failureCode: "FACT_GUARD_OUTPUT_INVALID", attempts: outcome.attempts };
  }
  ...
  const findings: FactGuardianFinding[] = raw.findings.map((finding) => ({
    ...
    generatedTextSha256: finding.generatedPath ? sha256(unitsByPath.get(finding.generatedPath)!.unit.text) : null,
    classification: finding.classification,
    sourceOrdinals: sortedUniqueNumbers(finding.sourceOrdinals),
    reasonCode: finding.reasonCode,
  }));
  const harmfulPaths = findings
    .filter((finding) => finding.generatedPath !== null && HARMFUL.has(finding.classification as "POSSIBLE_NEW_CLAIM" | "UNSUPPORTED_NEW_CLAIM" | "SOURCE_CONFLICT"))
  ...
  return { ok: true, pass, harmfulPaths: sortedUniqueStrings(harmfulPaths), attempts: outcome.attempts };
}

function sourceReplacement(unit: GeneratedResumeTextUnit, facts: readonly SourceFact[]): GeneratedResumeTextUnit | null {
  ...
  const candidates = facts.map((fact) => {
    ...
    .sort((a, b) => b.overlap - a.overlap || a.width - b.width || a.fact.text.localeCompare(b.fact.text));
  const selected = candidates[0]?.fact;
  return selected ? { text: selected.text, sourceRefs: [...selected.sourceRefs] } : null;
}

function repairDraft(source: CandidateResumeDocument, draft: GeneratedResumeDocument, harmfulPaths: readonly string[]) {
  ...
  const repairArray = (prefix: string, units: readonly GeneratedResumeTextUnit[]) => units
  ...
  const repairGroup = (prefix: string, group: GeneratedResumeListGroup): GeneratedResumeListGroup => ({
    ...group,
  ...
  const repaired = {
    ...draft,
    editorStatus: "PARTIAL_RECOVERY" as const,
    header: draft.header ? {
      ...draft.header,
    ...
    experience: draft.experience.map((entry, index) => repairEntry(`experience[${index}]`, entry)),
    projects: draft.projects.map((entry, index) => repairEntry(`projects[${index}]`, entry)),
    education: draft.education.map((entry, index) => repairEntry(`education[${index}]`, entry)),
    certifications: draft.certifications.map((entry, index) => repairEntry(`certifications[${index}]`, entry)),
    skillGroups: draft.skillGroups.map((group, index) => repairGroup(`skillGroups[${index}]`, group)),
    languageGroups: draft.languageGroups.map((group, index) => repairGroup(`languageGroups[${index}]`, group)),
    otherGroups: draft.otherGroups.map((group, index) => repairGroup(`otherGroups[${index}]`, group)),
  };
  return GeneratedResumeDocumentSchema.parse(repaired);
}
...
export async function guardAndRepairResume(
  ...
  if (!first.ok) return { ok: false, failureCode: first.failureCode, report: null, attempts: first.attempts };
  ...
  if (!second.ok) {
    ...
    return { ok: false, failureCode: second.failureCode, report, attempts: [...first.attempts, ...second.attempts] };
  }
  if (second.harmfulPaths.length > 0) {
    ...
    return { ok: false, failureCode: "FACT_GUARD_REJECTED", report, attempts: [...first.attempts, ...second.attempts] };
  }
  ...
  return { ok: true, document: repaired, report, attempts: [...first.attempts, ...second.attempts] };
}
