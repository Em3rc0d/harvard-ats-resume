...
function ensureReferenceProfile() {
  ...
  if (profile !== REQUIRED_PROFILE) {
    throw new Error(`Set CVENGINE_RUNTIME_PROFILE_ID=${REQUIRED_PROFILE}. Refusing to label another host as the ATS-SYS-03D reference runtime.`);
  }
}
...
function ensureCommittedSource() {
  ...
  if (sourceChanges.length > 0) {
    throw new Error(`ATS-SYS-03D requires committed source. Resolve: ${sourceChanges.join(' | ')}`);
  }
}
...
async function waitForReady(expectedBuildSha, timeoutMs = 120000) {
  ...
  throw new Error(`ATS-SYS-03D runtime did not become READY within ${timeoutMs} ms: ${last}`);
}
...
async function main() {
  ...
  const root = resolve(
    process.env.CVENGINE_SYS03D_REFERENCE_DIR
      || `evidence/ats-sys-03/model-forced-reference-runs/${isoSafe(startedAt)}`,
  );
  const capacityDir = resolve(root, 'model-forced-capacity');
  ...
  const env = {
    ...
    CVENGINE_SYS03D_EVIDENCE_DIR: capacityDir,
  };

  const manifest = {
    manifestVersion: 'ats-sys-03d-reference-run-v0.1',
    startedAt,
    completedAt: null,
    buildSha,
  ...
  try {
    ...
    for (const port of [APP_PORT, OLLAMA_PORT, REDIS_HTTP_PORT]) {
      ...
      if (owner) throw new Error(`ATS-SYS-03D reference port ${port} is occupied by ${owner}.`);
    }
    ...
    const harnessArgs = ['scripts/system-import-model-capacity.mjs', ...process.argv.slice(2)];
    run(process.execPath, harnessArgs, env);

    manifest.completedAt = new Date().toISOString();
    manifest.result = 'EVIDENCE_CAPTURED';
    await persistManifest();
    process.stdout.write(`\nATS-SYS-03D reference execution: EVIDENCE_CAPTURED\n`);
