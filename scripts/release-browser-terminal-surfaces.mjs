...
const context = {
  receipt: {
    receiptId: 'resume-import-browser-terminal',
...
const successfulResult = {
  ...
  claimTraceability: [],
  resumeVersion: {
    id: 'resume-version-browser-acceptance',
    contentSha256: 'b'.repeat(64),
    generation: { provider: 'fixture', model: 'fixture', contractVersion: 'fixture-v1' },
    createdAt: '2026-08-19T20:01:00.000Z',
  },
  resumePersistence: 'DURABLE_CAREER_VAULT',
  careerVault: {
    schemaVersion: 'career-vault-browser-v1',
    candidateProfileId: 'candidate-browser-acceptance',
    revision: 1,
...
async function installImportSuccessRoute() {
  await page.route('**/api/import-resume', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({ success: true, data: { resume, context } }),
    });
  });
}
...
async function reachGeneralTarget() {
  ...
  await page.locator('#cv-upload').setInputFiles({
    name: 'candidate.pdf',
    mimeType: 'application/pdf',
    buffer: Buffer.from('%PDF-1.4 terminal acceptance fixture'),
  });
...
try {
  ...
  // Grounding stop must be a navigable product state, not an exception or dead end.
  ...
  await page.route('**/api/generate-resume', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        success: false,
        error: 'Unsupported candidate wording requires confirmation.',
        grounding: { status: 'NEEDS_USER_CONFIRMATION', factsToConfirm: ['Led an unsupported enterprise program.'] },
      }),
    });
  });
  ...
  // Reset and prove the successful terminal surface plus all primary result actions.
