...
import { AlertTriangle, ArrowLeft, Compass, FileText, PencilLine, Search, ShieldCheck, Target } from 'lucide-react';
...
const COPY = {
  en: {
    ...
    readinessTitle: 'Your imported career needs review before assessment or generation.', readinessBody: 'Complete these source-backed fields before continuing:',
  ...
  es: {
    ...
    readinessTitle: 'Tu carrera importada necesita revisión antes de evaluar o generar.', readinessBody: 'Completa estos campos respaldados por fuente antes de continuar:',
  ...
  fr: {
    ...
    readinessTitle: 'Votre carrière importée doit être revue avant toute évaluation ou génération.', readinessBody: 'Complétez ces champs étayés avant de continuer :',
  ...
  pt: {
    ...
    readinessTitle: 'Sua carreira importada precisa de revisão antes de avaliar ou gerar.', readinessBody: 'Complete estes campos respaldados por fonte antes de continuar:',
...
export default function TargetJobStep({ data, isLoading, onBack, onEditDetails, onGenerate }: Readonly<TargetJobStepProps>) {
  ...
  const currentAssessment = assessment &&
    assessedJobDescription === normalizedJobDescription &&
    assessedTargetKey === targetKey
    ? assessment
    : null;
  ...
  const canAssess = useMemo(() => (
    !isLoading && !isAssessing && mode === 'TARGETED' && readiness.ready &&
    normalizedJobDescription.length >= 20 && normalizedTargetRole.length >= 2
  ), [isAssessing, isLoading, mode, normalizedJobDescription, normalizedTargetRole, readiness.ready]);

  const canGenerate = useMemo(() => {
    if (isLoading || isAssessing || mode === null || !readiness.ready) return false;
    if (mode === 'GENERAL') return true;
    return Boolean(currentAssessment && targetRelevance);
  }, [currentAssessment, isAssessing, isLoading, mode, readiness.ready, targetRelevance]);
  ...
  const assess = async () => {
    ...
    try {
      ...
      setAssessedTargetKey(targetKey);
    } catch (error) {
    ...
    <div className="mx-auto max-w-4xl space-y-6">
        ...
        {!readiness.ready && (
          <div className="mt-6 rounded-xl border border-amber-300 bg-amber-50 p-4">
            <div className="flex gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-700" /><div>
              ...
              <div className="mt-3 space-y-1.5">{readiness.issues.slice(0, 8).map((issue) => (
                <p key={`${issue.fieldPath}-${issue.message}`} className="rounded-md border border-amber-200 bg-white/70 px-3 py-2 text-xs text-amber-950"><span className="font-mono font-bold">{issue.fieldPath}</span> — {issue.message}</p>
              ))}</div>
              <button type="button" onClick={onEditDetails} className="mt-4 inline-flex items-center gap-2 rounded-md bg-amber-900 px-4 py-2 text-sm font-bold text-white hover:bg-amber-800"><PencilLine className="h-4 w-4" /> {copy.edit}</button>
            </div></div>
          </div>
        )}

        <div className="mt-7 grid gap-4 md:grid-cols-2">
          <button type="button" onClick={() => selectMode('TARGETED')} className={`rounded-xl border-2 p-5 text-left transition ${mode === 'TARGETED' ? 'border-blue-600 bg-blue-50' : 'border-gray-200 bg-white hover:border-gray-400'}`}>
            <Target className="h-5 w-5 text-blue-700" /><p className="mt-3 font-bold text-gray-950">{copy.targeted}</p><p className="mt-1 text-sm leading-relaxed text-gray-500">{copy.targetedDesc}</p>
          </button>
          <button type="button" onClick={() => selectMode('GENERAL')} className={`rounded-xl border-2 p-5 text-left transition ${mode === 'GENERAL' ? 'border-gray-950 bg-gray-50' : 'border-gray-200 bg-white hover:border-gray-400'}`}>
            <FileText className="h-5 w-5 text-gray-700" /><p className="mt-3 font-bold text-gray-950">{copy.general}</p><p className="mt-1 text-sm leading-relaxed text-gray-500">{copy.generalDesc}</p>
          </button>
        </div>
        ...
        {mode === 'TARGETED' && (
          <div className="mt-6 space-y-6">
            <div className="rounded-xl border border-violet-200 bg-violet-50/60 p-5">
              <div className="flex items-start gap-3"><Compass className="mt-0.5 h-5 w-5 shrink-0 text-violet-700" /><div><h3 className="font-bold text-violet-950">{copy.targetTitle}</h3><p className="mt-1 text-xs leading-relaxed text-violet-800">{copy.targetHelp}</p></div></div>
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <label className="text-sm font-semibold text-gray-800">{copy.roleLabel}<input value={targetRole} onChange={(event) => { setTargetRole(event.target.value); invalidateAssessment(); }} placeholder={copy.rolePlaceholder} className="mt-2 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-normal outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-100" /></label>
                <label className="text-sm font-semibold text-gray-800">{copy.seniorityLabel}<select value={targetSeniority} onChange={(event) => { setTargetSeniority(event.target.value as TargetSeniority); invalidateAssessment(); }} className="mt-2 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-normal">{SENIORITIES.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
                <label className="text-sm font-semibold text-gray-800">{copy.locationLabel}<input value={targetLocation} onChange={(event) => { setTargetLocation(event.target.value); invalidateAssessment(); }} placeholder={copy.locationPlaceholder} className="mt-2 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-normal outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-100" /></label>
                <label className="text-sm font-semibold text-gray-800">{copy.workModelLabel}<select value={targetWorkModel} onChange={(event) => { setTargetWorkModel(event.target.value as TargetWorkModel); invalidateAssessment(); }} className="mt-2 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-normal">{WORK_MODELS.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
              </div>
            </div>

            <div>
              <label htmlFor="target-job-description" className="text-sm font-semibold text-gray-800">{copy.label}</label>
              <textarea id="target-job-description" value={jobDescription} onChange={(event) => { setJobDescription(event.target.value); invalidateAssessment(); }} rows={14} className="mt-2 w-full rounded-lg border border-gray-300 px-4 py-3 text-sm leading-relaxed outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" placeholder={copy.placeholder} />
              <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center">
                <button type="button" onClick={assess} disabled={!canAssess} className="inline-flex items-center justify-center gap-2 rounded-md bg-blue-700 px-5 py-2.5 text-sm font-bold text-white hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-40"><Search className="h-4 w-4" />{isAssessing ? copy.assessing : copy.assess}</button>
                <p className="text-xs leading-relaxed text-gray-500">{copy.assessHint}</p>
              </div>
              {assessmentError && <div className="mt-4 rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm text-rose-800">{assessmentError}</div>}
            </div>
          </div>
        )}

        <div className="mt-6 rounded-lg border border-blue-200 bg-blue-50 p-4"><div className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-blue-700" /><p className="text-sm leading-relaxed text-blue-900">{copy.trust}</p></div></div>
      </section>
      ...
      {currentAssessment && targetRelevance && (
        <section className="rounded-2xl border border-violet-200 bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-violet-600">Career Target</p><h3 className="mt-1 text-xl font-bold text-gray-950">{copy.relevanceTitle}</h3></div><span className="rounded-full border border-violet-200 bg-violet-50 px-3 py-1 text-xs font-black text-violet-800">{targetRelevance.level}</span></div>
          <div className="mt-4 grid grid-cols-2 gap-2 text-xs md:grid-cols-5">{([
            ['ROLE', targetRelevance.role], ['SENIORITY', targetRelevance.seniority], ['LOCATION', targetRelevance.location], ['WORK MODEL', targetRelevance.workModel], ['EMPLOYMENT', targetRelevance.employmentType],
          ] as const).map(([label, status]) => <div key={label} className="rounded-lg border border-gray-200 p-3"><p className="font-bold text-gray-500">{label}</p><p className="mt-1 font-black text-gray-900">{status}</p></div>)}</div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <button type="button" onClick={onBack} disabled={isLoading || isAssessing} className="inline-flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-900 disabled:opacity-50"><ArrowLeft className="h-4 w-4" /> {copy.back}</button>
        <button type="button" onClick={onEditDetails} disabled={isLoading || isAssessing} className="inline-flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-900 disabled:opacity-50 sm:ml-3"><PencilLine className="h-4 w-4" /> {copy.edit}</button>
        <button type="button" onClick={generate} disabled={!canGenerate} className="rounded-md bg-gray-950 px-6 py-3 text-sm font-bold text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-40 sm:ml-auto">{isLoading ? copy.generating : mode === 'TARGETED' ? copy.build : copy.generate}</button>
      </div>
    </div>
  );
}
