...
function completeEvidence() {
  return [
    ...
    { fieldPath: 'experience[0].endDate', excerpt: '2025', page: 1 },
    {
      ...
      excerpt: 'Built TypeScript APIs for internal workflows.',
      page: 1,
    },
...
test('native import rejects candidate values without source evidence', () => {
  ...
  assert.throws(
    () => validateAndMapEvidence(candidateFixture(), evidence, documentFixture()),
    /missing source evidence for experience\[0\]\.role/,
  );
});
...
test('native import rejects evidence excerpts that are absent from the source document', () => {
  const evidence = completeEvidence().map((item) =>
    item.fieldPath === 'experience[0].role'
      ? { ...item, excerpt: 'Principal Architect' }
      : item,
  );

  assert.throws(
    () => validateAndMapEvidence(candidateFixture(), evidence, documentFixture()),
    /evidence is not present in source text for experience\[0\]\.role/,
  );
});

test('legacy binary DOC is rejected rather than routed through an opaque converter', () => {
  assert.throws(
    () => resolveResumeMimeType('candidate.doc', 'application/msword'),
    /Use PDF or DOCX/,
  );
});
