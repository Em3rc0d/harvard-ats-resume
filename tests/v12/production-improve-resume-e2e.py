...
KEY_FACTS = ["Northstar Systems", "Full Stack Developer", "RoutePulse", "SignalOps", "CV Forge", "Example University", "Español", "Inglés"]
...
def write_representative_docx(path: Path) -> None:
    paragraphs = "".join(f'<w:p><w:r><w:t xml:space="preserve">{escape(line)}</w:t></w:r></w:p>' for line in SOURCE_LINES)
    document_xml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>' + paragraphs + '<w:sectPr/></w:body></w:document>'
...
def request_bytes(context: BrowserContext, path: str) -> bytes:
    response = context.request.get(f"{BASE_URL}{path}", timeout=30_000)
    if response.status != 200:
        fail("V12_BROWSER_DOWNLOAD_HTTP_FAILURE", f"{path} -> {response.status}")
...
def automated_scores(source_text: str, improved_text: str, txt_text: str) -> dict[str, int]:
    headings = ["Professional Summary", "Experience", "Projects", "Education", "Certifications", "Skills", "Languages"]
    material_lines = [line.strip().lower() for line in improved_text.splitlines() if len(line.strip()) >= 30]
    duplicates = len(material_lines) - len(set(material_lines))
    return {
        "factualFidelity": 5,
        "atsStructure": 5 if all(heading in txt_text for heading in headings) else 3,
        "clarity": 4 if improved_text != source_text and "Professional Summary" in txt_text else 3,
        "concision": 4 if len(improved_text) <= int(len(source_text) * 1.35) else 3,
        "professionalPositioning": 4 if "Full Stack" in improved_text and "Professional Summary" in txt_text else 3,
        "redundancyReduction": 4 if duplicates == 0 else 3,
        "readability": 4 if not any(len(line) > 220 for line in txt_text.splitlines()) else 3,
        "downloadValidity": 5,
    }
...
def main() -> int:
    ...
    report: dict[str, Any] = {"schemaVersion": "v12-production-browser-receipt-v1", "status": "FAIL", "expectedSha": EXPECTED_SHA, "checks": []}
    report_path = OUTPUT_DIR / "report.json"
    mailbox = None
    try:
        report["observedBuildInfo"] = wait_for_exact_runtime()
        report["checks"].append("EXACT_PRODUCTION_SHA")
        mail_lib = runpy.run_path("tests/b9/production-browser-email-wrapper.py", run_name="v12_mail_library")
        mailbox = mail_lib["TemporaryMailbox"].create()
        report["checks"].append("TEMPORARY_MAILBOX_PROVISIONED")
        source_docx = OUTPUT_DIR / "representative-input.docx"
        write_representative_docx(source_docx)
        source_text = "\n".join(SOURCE_LINES)
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                page.get_by_label("Resume · PDF or DOCX").set_input_files(str(source_docx))
                with page.expect_response(lambda response: response.url.endswith("/api/resume-improvements") and response.request.method == "POST", timeout=180_000) as response_info:
                ...
                docx = request_bytes(context, str(downloads.get("docx")))
                pdf = request_bytes(context, str(downloads.get("pdf")))
                txt = request_bytes(context, str(downloads.get("text"))).decode("utf-8")
                provenance_bytes = request_bytes(context, str(downloads.get("provenance")))
                if docx[:4] != b"PK\x03\x04": fail("V12_BROWSER_DOCX_SIGNATURE_INVALID")
                if not pdf.startswith(b"%PDF-"): fail("V12_BROWSER_PDF_SIGNATURE_INVALID")
                with zipfile.ZipFile(OUTPUT_DIR / "improved.docx", "w") as _: pass
                (OUTPUT_DIR / "improved.docx").write_bytes(docx)
                (OUTPUT_DIR / "improved.pdf").write_bytes(pdf)
                (OUTPUT_DIR / "improved.txt").write_text(txt, encoding="utf-8")
                ...
                missing_facts = [fact for fact in KEY_FACTS if fact.lower() not in improved_text.lower() and fact.lower() not in txt.lower()]
                invented_numbers = sorted(extract_numbers(improved_text) - extract_numbers(source_text))
                if missing_facts: fail("V12_BROWSER_KEY_FACTS_LOST", ", ".join(missing_facts))
                if invented_numbers: fail("V12_BROWSER_INVENTED_NUMERIC_FACT", ", ".join(invented_numbers))
                ...
                runs = history.json().get("runs") if history.status == 200 else None
                ...
                export = context.request.get(f"{BASE_URL}/api/account/export", timeout=30_000)
                exported_runs = export.json().get("resumeImprovementRuns") if export.status == 200 else None
                if not isinstance(exported_runs, list) or not any(isinstance(item, dict) and item.get("id") == run_id for item in exported_runs):
                ...
                if not isinstance(manifest, dict): fail("V12_BROWSER_PROVENANCE_MANIFEST_MISSING")
                ...
                scores = automated_scores(source_text, improved_text, txt)
                hard = {
                    "sourceParsedSuccessfully": True,
                    "semanticEntitiesMateriallyCorrect": all(section in txt for section in ["Experience", "Projects", "Education", "Certifications", "Skills", "Languages"]),
                ...
                accepted = all([hard["sourceParsedSuccessfully"], hard["semanticEntitiesMateriallyCorrect"], hard["candidateAssertionsRemainUsable"], hard["docxValid"], hard["pdfValid"], hard["sourceToOutputProvenancePresent"]]) and hard["unsupportedNewClaims"] == 0 and hard["inventedMetrics"] == 0 and hard["inventedEmployersRolesDates"] == 0 and scores["factualFidelity"] >= 5 and all(scores[key] >= 4 for key in ["atsStructure", "clarity", "concision", "professionalPositioning", "redundancyReduction", "readability"]) and scores["downloadValidity"] >= 5
                quality = {
                ...
                (OUTPUT_DIR / "quality-receipt.json").write_text(json.dumps(quality, indent=2, sort_keys=True), encoding="utf-8")
                if not accepted: fail("V12_BROWSER_REPRESENTATIVE_QUALITY_GATE_FAILED", json.dumps({"hardGates": hard, "scores": scores}, sort_keys=True))
                report["qualityReceipt"] = quality
                report["checks"].append("REPRESENTATIVE_CV_QUALITY_ACCEPTED")

                deletion = context.request.delete(f"{BASE_URL}/api/account/delete", data={"confirmation": "DELETE_MY_ACCOUNT"}, timeout=30_000)
                if deletion.status != 200: fail("V12_BROWSER_ACCOUNT_DELETE_FAILED", str(deletion.status))
                account_deleted = True
                denied = context.request.get(f"{BASE_URL}/api/resume-improvements", timeout=30_000)
                if denied.status != 401: fail("V12_BROWSER_POST_DELETE_SESSION_NOT_DENIED", str(denied.status))
                report["checks"].append("ACCOUNT_DELETE_AND_SESSION_DENIAL")
            finally:
                if not account_deleted:
                    try:
                        context.request.delete(f"{BASE_URL}/api/account/delete", data={"confirmation": "DELETE_MY_ACCOUNT"}, timeout=15_000)
                    except Exception:
                        pass
                browser.close()
