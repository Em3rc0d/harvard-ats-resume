...
-- B9 planner v3 replaces UUID-shaped density selection with an explainable,
-- source-order-aware section budget. Historical v1/v2 plans remain valid.
alter table public.resume_plans drop constraint resume_plans_planner_version_check;
alter table public.resume_plans
  add constraint resume_plans_planner_version_check
  check (planner_version in (
    'b9-deterministic-resume-plan-v1',
    'b9-deterministic-resume-plan-v2',
    'b9-deterministic-resume-plan-v3'
  ));
...
begin
  ...
  with candidates as (
    ...
    select e.*,
      ...
      case e.section
        ...
        when 'CERTIFICATIONS' then 3
        when 'SKILLS' then 4
        when 'LANGUAGES' then 1
        else 0
      end as section_budget,
      row_number() over (
        partition by e.section
        order by e.target_priority, e.source_ordinal, e.source_created_at, e.evidence_id::text
      ) as section_slot
    from eligible e
  ), selected as (
    select *
    from ranked
    where section_slot <= section_budget
    order by section_priority, target_priority, source_ordinal, source_created_at, evidence_id::text
    limit 20
  )
  select coalesce(jsonb_agg(
    ...
    ) order by s.section_priority, s.target_priority, s.source_ordinal, s.source_created_at, s.evidence_id::text
  ), '[]'::jsonb)
  into v_items
  from selected s
  left join lateral (
  ...
  v_semantic_key := public.cv_engine_sha256(
    ...
    || 'b9-deterministic-resume-plan-v3' || chr(31)
