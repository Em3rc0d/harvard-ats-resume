...
-- A grouped import block must be contiguous in the original extracted document,
-- not merely contiguous after proposal de-duplication/normalization. source_line is
-- the authoritative structural coordinate; proposal ordinal is only review order.
...
declare
  ...
  v_pending integer;
  v_receipts integer;
  v_min_source_line integer;
  v_max_source_line integer;
  v_receipt_id uuid;
...
begin
  ...
  select count(*),
    ...
    into v_total, v_pending, v_receipts, v_min_source_line, v_max_source_line, v_canonical_text
  from public.import_proposals ip
  where ip.owner_user_id = v_owner
    and ip.id = any(p_proposal_ids);
  ...
  if v_max_source_line - v_min_source_line + 1 <> v_total then
    raise exception 'B5_IMPORT_GROUP_SOURCE_LINES_NONCONTIGUOUS' using errcode = '23514';
  end if;
