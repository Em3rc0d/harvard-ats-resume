...
-- Historical presentation is immutable during normal product use, but privacy/account
-- lifecycle remains authoritative. Direct DELETE is already unavailable to API roles,
-- so delete-blocking triggers would incorrectly prevent legitimate account erasure.
drop trigger if exists presentation_plans_no_delete on public.presentation_plans;
drop trigger if exists presentation_plan_evidence_no_delete on public.presentation_plan_evidence;
drop trigger if exists presentation_revisions_no_delete on public.presentation_revisions;
drop trigger if exists presentation_revision_evidence_no_delete on public.presentation_revision_evidence;
...
begin
  ...
  return jsonb_build_object(
    'schemaVersion', 'p1-account-export-v2',
    'ownerUserId', v_owner,
    'exportedAt', now(),
    ...
    'presentationPlans', (select coalesce(jsonb_agg(to_jsonb(t) order by t.id), '[]'::jsonb) from public.presentation_plans t where t.owner_user_id = v_owner),
    'presentationPlanEvidence', (select coalesce(jsonb_agg(to_jsonb(t) order by t.plan_id, t.selection, t.ordinal), '[]'::jsonb) from public.presentation_plan_evidence t where t.owner_user_id = v_owner),
    'presentationRevisions', (select coalesce(jsonb_agg(to_jsonb(t) order by t.id), '[]'::jsonb) from public.presentation_revisions t where t.owner_user_id = v_owner),
    'presentationRevisionEvidence', (select coalesce(jsonb_agg(to_jsonb(t) order by t.presentation_revision_id, t.ordinal), '[]'::jsonb) from public.presentation_revision_evidence t where t.owner_user_id = v_owner)
  );
end;
$$;
...
begin
  ...
  -- P1 first because its provenance intentionally RESTRICTs deletion of source truth.
  delete from public.presentation_revision_evidence where owner_user_id = v_owner;
  delete from public.presentation_revisions where owner_user_id = v_owner;
  delete from public.presentation_plan_evidence where owner_user_id = v_owner;
  delete from public.presentation_plans where owner_user_id = v_owner;
...
-- CREATE OR REPLACE preserves the B8 privileges, but assert intent explicitly.
revoke all on function public.cv_engine_export_account() from public, anon;
revoke all on function public.cv_engine_delete_account() from public, anon;
