...
create table public.import_review_structures (
  id uuid primary key default gen_random_uuid(),
  receipt_id uuid not null,
  owner_user_id uuid not null,
  structure_version text not null check (structure_version = 'v1.1-ai-import-structure-v1'),
  status text not null check (status in ('AI_STRUCTURED','HYBRID','DETERMINISTIC_FALLBACK')),
  blocks jsonb not null check (
    jsonb_typeof(blocks) = 'array'
    and jsonb_array_length(blocks) between 1 and 100
  ),
  ai_runs jsonb not null default '[]'::jsonb check (
    jsonb_typeof(ai_runs) = 'array'
    and jsonb_array_length(ai_runs) between 0 and 20
  ),
  created_at timestamptz not null default now(),
  constraint import_review_structures_receipt_unique unique (receipt_id),
  constraint import_review_structures_receipt_owner_fk foreign key (receipt_id, owner_user_id)
    references public.import_receipts(id, owner_user_id) on delete cascade
);

create index import_review_structures_owner_created_idx
  on public.import_review_structures(owner_user_id, created_at desc);

create or replace function public.cv_engine_guard_import_review_structure_update()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  raise exception 'V11_IMPORT_REVIEW_STRUCTURE_IMMUTABLE' using errcode = '23514';
end;
$$;

create trigger import_review_structures_immutable
before update on public.import_review_structures
for each row execute function public.cv_engine_guard_import_review_structure_update();

alter table public.import_review_structures enable row level security;

create policy "import_review_structures_select_own"
on public.import_review_structures
for select
to authenticated
using ((select auth.uid()) = owner_user_id);

create policy "import_review_structures_insert_own"
on public.import_review_structures
for insert
to authenticated
with check ((select auth.uid()) = owner_user_id);

revoke all on table public.import_review_structures from anon, authenticated;
grant select, insert on table public.import_review_structures to authenticated;
revoke all on function public.cv_engine_guard_import_review_structure_update() from public, anon, authenticated;
